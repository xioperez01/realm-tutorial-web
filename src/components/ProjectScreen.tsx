...
function useDraftTask({ addTask }: { addTask: any }) {
  ...
  const setDraftTaskName = (name: any) => {
    setDraftTask({ name });
  };
...
function TaskList({ currentProject }: { currentProject: any }) {
  const { tasks, addTask, loading } = useTasks(currentProject);
  const getTaskById = (id: any) => tasks.find((task: any) => task._id === id);
